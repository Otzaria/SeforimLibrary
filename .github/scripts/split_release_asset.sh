#!/usr/bin/env bash
# Split one archive into GitHub-uploadable parts plus a manifest.
#
# GitHub refuses a release asset of 2 GiB or more, and the library search
# index compresses to several GiB. The manifest schema here is byte-for-byte
# the one Otzaria/otzaria consumes with tool/release/assemble_split_asset.sh
# (and .ps1) — a reader on the other side already exists, so do not change the
# field names without changing that repository in the same cycle.
set -euo pipefail
