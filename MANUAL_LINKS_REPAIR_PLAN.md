...
כדי שסחיפה שנספגה לא תעבור בשקט גם בשער האוטומטי, `ManualLinksCorpusMain` דורש בנוסף
`anchors.unrelocatable == expectedUnrelocatableAnchors` — ציפייה מפורשת וחובה, כשאר המונים
המוצמדים; `manual-links-corpus-qa.yml` מצמיד אותה ל־`0`, ולכן קבלה מודעת של רשומות
בלתי־ניתנות־להעברה מחייבת שינוי מפורש של הערך המוצמד, ולא מספיקה לה שורת `::warning::`.
